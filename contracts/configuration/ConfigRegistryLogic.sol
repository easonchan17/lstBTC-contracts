...
contract ConfigRegistryLogic is IConfigRegistry, AccessControlBase, UUPSUpgradeable {
    // Thrown when custodian ID is not authorized
    error UnauthorizedCustodian(uint32 custodianId);

    // Thrown when Bitcoin confirmations are below minimum requirement
    error BitcoinConfirmationsBelowMinimum(uint32 confirmations);

    // Thrown when native confirmations are below minimum requirement
    error NativeConfirmationsBelowMinimum(uint32 confirmations);

    // Thrown when redeem dust amount is below minimum
    error RedeemDustAmountBelowMinimum(uint64 amount);

    // Thrown when treasury fee rate exceeds maximum allowed
    error TreasuryFeeRateExceedsMax(uint16 rate);

    // Thrown when recipients array is empty
    error RecipientsEmpty();

    // Thrown when recipients and shares arrays have different lengths
    error RecipientsSharesLengthMismatch(uint256 recipientCount, uint256 shareCount);

    // Thrown when total shares do not equal the expected base amount
    ...
    // Default minimum redeem dust amount in satoshis
    ...
    // Fee configuration for peg-out operations
